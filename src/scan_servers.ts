...
import { DarknetServer } from "./helpers";
...
export async function main(ns: NS) {
  ...
  const host_map: Record<string, Server | DarknetServer> = {};
  for (const v of db.data) {
    if (v.server === null) continue;
    host_map[v.server.hostname] = v.server;
  }
  for (const info of db.data) {
    const host = info.server.hostname;
    const server = (info as HostInfo).server;
    if (!server) {
