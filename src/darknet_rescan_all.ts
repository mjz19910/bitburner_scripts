...
export async function main(ns: NS) {
	...
	ns.tprint(`[rescan] started=${started} skipped=${skipped} failed=${failed}`)
}
...
function loadKnownNodes(ns: NS): string[] {
	...
	if (ns.fileExists(TOPOLOGY_FILE, "home")) {
		try {
			const topo = JSON.parse(String(ns.read(TOPOLOGY_FILE))) as Topology
			for (const node of Object.keys(topo.nodes ?? {})) {
				found.add(node)
			}
		} catch (err) {
			ns.tprint(`[rescan] failed to parse ${TOPOLOGY_FILE}: ${String(err)}`)
		}
	}
	...
	if (found.size === 0 && ns.fileExists(MAP_FILE, "home")) {
		try {
			const raw = JSON.parse(String(ns.read(MAP_FILE))) as Record<string, Report>
