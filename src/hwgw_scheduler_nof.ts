...
async function fillRunner(ns: NS, targets: string[], runner: string) {
	...
	for (let i = 0; i < targets.length; i++) {
		...
		const result = await scheduleDynamicHWGW(
			ns,
			target,
			runner,
			batchId,
	...
	while (true) {
		while ((msg = c.readOrUndefined())) {
			...
			if (x.type === "hack") {
				if (r.data > 0.01) {
					ns.tprint(
						`[${r.server}] batch=${r.batchId} target=${r.target} hack=$${
							ns.format.number(r.data, 2)
						}`,
					);
				}
			} else if (x.type === "grow") {
				...
				if (r.data > 1.000001) {
					ns.tprint(
						`[${r.server}] batch=${r.batchId} target=${r.target} grow=${
							ns.format.number(r.data, 8)
						}`,
					);
				}
			} else if (x.type === "weaken") {
				const r = x.data;
				if (r.data > 0.0001) {
					ns.tprint(
						`[${r.server}] batch=${r.batchId} target=${r.target} weaken=${
							ns.format.number(r.data, 4)
						}`,
					);
				}
			}
...
class HWGWDelays {
	...
	constructor(ns: NS, target: string) {
		...
		this.allFinish = Math.max(
			this.hackFinish,
			this.weakenHackFinish,
			this.growFinish,
			this.weakenGrowFinish,
		);
	}
	...
	shiftEnd(delta: number) {
		...
		this.weakenHackDelay = Math.max(
			0,
			this.weakenHackFinish - weakenHackTime,
		);
		this.growDelay = Math.max(0, this.growFinish - growTime);
		this.weakenGrowDelay = Math.max(
			0,
			this.weakenGrowFinish - weakenGrowTime,
		);
	}
}
...
async function scheduleDynamicHWGW(
	...
	if (
		ns.getServerRequiredHackingLevel(target) > ns.getHackingLevel() * 1.15
	) return null;
	...
	const launched = await launchHWGWTasksStrict(
		ns,
		server,
		batchId,
		target,
