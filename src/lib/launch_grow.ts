import { build_script_args } from "helpers/args"
import { add_tag, omit_default } from "helpers"
import { GrowArgs } from "helpers/grow_args"
...
export function launchGrowSomewhere(
	...
	const growPortNum = opts.portNum ?? 1
	...
	for (const host of allServers) {
		...
		if (verbose) ns.tprint(`Launching ${threadsCanRun} threads on ${host} to grow ${target}`)
		...
		const args = build_script_args<GrowArgs>({
			_: [add_tag("runner", host), add_tag("threads", threadsCanRun), add_tag("target", target)],
			port: omit_default(growPortNum, 1),
		})
