import { HostInfoDB } from "./types/HostInfoDB"
...
function copy_file_to({
	...
	if (idx !== -1) {
		...
		if (!new_copy_to_servers.includes(dst.host)) new_copy_to_servers.push(dst.host)
...
export async function main(ns: NS) {
	...
	for (const { host, server: srv } of db.data) {
		...
		if (host !== "home") {
			if (!srv!.purchasedByPlayer) {
				for (const file of files) {
					...
					if (file.endsWith(".json") || file.startsWith("old/")) {
						ns.rm(file, host)
						if (!rm_old_servers.includes(host)) rm_old_servers.push(host)
						if (!rm_old_files.includes(file)) rm_old_files.push(file)
			...
			for (const file of home_files) {
				...
				if (file.endsWith(".json")) {
					if (srv!.purchasedByPlayer) {
						copy_file_to({ ns, src, dst: dstTarget, file, new_copy_to_files, new_copy_to_servers })
					}
					continue
				}
		...
		if (db.host_index.has("worker-1")) {
			...
			if (host !== "worker-1") {
				for (const file of files) {
					...
					ns.tprint("copy lit ", file, " from ", host, " to worker-1")
