const PROBE_WORKER = "darknet_probe_worker.ts"
const ORCHESTRATOR_PORT = 5

const static_known_passwords = [
	"admin",
	"password",
	"0000",
	"12345",
...
export async function main(ns: NS) {
	...
	if (targets.length === 0) {
		ns.tprint(`[${host}] usage: run darknet_expand_worker.ts <target1> [target2 ...]`)
		return
	}
...
async function expandOne(ns: NS, source: string, target: string) {
	...
	for (const pw of candidates) {
		try {
			...
			ns.print(`[${source}] authenticate(${target}, "${pw}") => ${JSON.stringify(result)}`)
			...
			ns.print(`[${source}] authenticate(${target}, "${pw}") threw: ${String(err)}`)
	...
	try {
		...
		ns.print(`[${source}] connectToSession(${target}) => ${JSON.stringify(conn)}`)
		...
		ns.print(`[${source}] connectToSession(${target}) threw: ${String(err)}`)
		return
	}
...
function getAuth(ns: NS, source: string, target: string): AuthInfo | null {
	try {
		...
		ns.print(`[${source}] getServerAuthDetails(${target}) failed: ${String(err)}`)
		return null
	}
}
...
function buildPasswordCandidates(authInfo: AuthInfo): string[] {
	...
	if ((authInfo.passwordFormat ?? "").toLowerCase() === "numeric") {
		out.add("0")
		out.add("00")
		out.add("000")
		out.add("0000")
...
function report(
	...
	ns.tryWritePort(ORCHESTRATOR_PORT, JSON.stringify({
		source,
		target,
		authInfo,
		password,
...
async function tryHeartbleed(ns: NS, target: string) {
	try {
		...
		ns.print(`[${ns.getHostname()}] heartbleed(${target}) logs:\n${hb.logs.join("\n")}`)
	} catch {}
}
